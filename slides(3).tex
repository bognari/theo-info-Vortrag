...
\begin{document}
  ...
  \begin{frame} %Stephan
    ...
    Also jedes Tupel von $\{0, 1\}$ Elementen wird genau auf ein $\{0, 1\}$ Element abgebildet.
  \end{frame}
  ...
  \begin{frame}
    \begin{figure}
      ...
      Die Ordnungsrelation ist NICHT mehr einfach von Unten nach Oben ablesbar, weil das Tupel $101$ und $010$ nicht die 3 partiellen Ordnungen erfüllen.
    \end{figure}
  \end{frame}
  ...
  \begin{frame}
    ...
    \begin{itemize}
      ...
      \item $S(f)$ "kleinsten" möglichen Schaltkreis für $f(x)$
      \item $C_M(f) = min \{G(S_M(f))\}$
      \item $S_M(f)$ "kleinsten" möglichen monotonen Schaltkreis $f(x)$
    \end{itemize}
  \end{frame}
  ...
  \begin{frame}
    ...
    \end{thm}
    Das heißt also: Je größer ein Schaltkreis ist, umso größer wächst der Abstand der Komplexitäten.
    Dieses Wachsen verhält sie wie eine Exponentialfunktion.\\
  \end{frame}
  ...
  \begin{frame}%Marko
    ...
    Das $k$ gibt hier an wie viele Eing\"ange auf $1$ stehen mussten damit der Ausgang $1$ wurde.
  \end{frame}
  ...
  \begin{frame}
    ...
    \begin{defin}
      ...
      \end{align*}
    \end{defin}
    Durch diese Definition ist der $k-te$ Slice von jeder booleschen Funktion immer monoton.
  \end{frame}
  ...
  \begin{frame}%Johanes
    ...
    \begin{lem}
      Für jeden $U_2$ (nicht monotonen) Schaltkreis $\beta$ gibt es einen äquivalenten Schaltkreis, welcher höchstens
      2 mal so groß ist wie $\beta$, bei dem die Verneinungen durch die Bereitstellung der beiden Zustände der Eingänge also $1$ und $0$ realisiert werden. %9.4 Johanes
    \end{lem}
